...
class Test::Mock::Log {
    ...
    method log-method-call($name, $capture) is protected {
        @!log-entries.push($%( :$name, :$capture ));
    }

    method called($name, :$times, :$with) is protected {
    ...
    method never-called($name, :$with) is protected {
            ...
            for $p.^method_table.kv -> $method-name, $method-obj {
                unless %already-seen{$method-name} {
                    my $meth = method (|c) is rw {
                        ...
                        elsif %returning{$method-name} ~~ Iterable {  # UNCOVERABLE
